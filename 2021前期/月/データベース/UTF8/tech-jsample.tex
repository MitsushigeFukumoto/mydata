...
\title{データベース及び演習2021中間レポート\\
（2021年6月14日版）}

\etitle{Detabases and Exercise2021 Mid-term Report \\ (version 2021/06/14)}

\affiliate{IPSJ}{データベース中間レポート\\
IPSJ, Chiyoda, Tokyo 101--0062, Japan}


\paffiliate{JU}{愛知工業大学\\
Aichi institute of technology Uniersity}

\author{K19093 福本　光重}{Fukumoto Mitsushige}{IPSJ}[k19093kk@aitech.ac.jp]

\begin{abstract}
本稿は、データベース及び演習2021の中間レポートである。データベース及び演習のこれまでの授業をPHP7+MariaDB/MySQlマスターブック
掲載のサンプルプログラムを元に、システムの概要をまとめた。
機能概要、利用技術、システム設計、実装、まとめの順にサンプルプログラム
の仕様を解説する。
...
本レポートで取り扱っているサンプルプログラムは、PHPとMySQLを用いた
会員管理システムである。
...
PHPとは、Personal Home Pageが由来のプログラミング言語である。
正式名称はPHP:HyperText Preprocessorという。Webで利用されるHTML形式のようなハイパーテキストを閲覧者の操作によって生成して、
動的な画面を作るのが得意である。会員制システム、通販システムやEラーニングなどのWebアプリケーション開発
する状況では実行速度や実装の容易さからよく利用されている言語の１つとなっている。
本レポートで取り扱うサンプルプログラムでは、利用者側（会員側）に実際に表示される画面の部分を担っている。
特徴としては、
\begin{itemize}
	\item {}
	\item {}
\end{itemize}
PHPはwebサーバー上で動作するサーバーサイドスクリプト言語であり、
Webで利用されるHTML形式のような
